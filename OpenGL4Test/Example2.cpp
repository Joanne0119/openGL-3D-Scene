...
glm::vec3 g_eyeloc(8.0f, 8.0f, 8.0f); // 鏡頭位置, 預設在 (8,8,8) 
CCube g_centerloc; // view center預設在 (0,0,0)，不做任何描繪操作
CQuad g_floor[ROW_NUM][ROW_NUM]; // 64 個四邊形組成的地板

GLuint g_shadingProg;

// 全域光源 (位置在 4,5,0)
CLight g_light(glm::vec3(4.0f, 5.0f, 0.0f));
CCube  g_spotTarget; // spot light 的 target 預設在 (0,0,0)，不做任何描繪操作
...
{
    genMaterial();  // 建立材質
    g_shadingProg = CShaderPool::getInstance().getShader("v_gouraud.glsl", "f_gouraud.glsl");

    //設定 Spot light，target 指向 (0,0,0)
    g_spotTarget.setPos(glm::vec3(0, 0, 0));
    g_light.setShaderID(g_shadingProg, "uLight");
    g_light.setTarget(g_spotTarget.getPos());
    g_light.setCutOffDeg(20.0f, 40.0f);
//  g_light.setCutOffDeg(20.0f, 60.0f, 8.0f);
    ...
    g_teapot.setScale(glm::vec3(0.5f, 0.5f, 0.5f));
    g_teapot.setPos(glm::vec3(1.0f, 0.0005f, -1.0f));
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "OpenGL_4 Example 2 Spot Light", nullptr, nullptr);
    if (!window) {
